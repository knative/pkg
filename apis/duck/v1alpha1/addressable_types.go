...
// Hostname is deprecated as of v0.6 and replaced with URL. It will be removed in ~v0.7
// URL string currently supports scheme(http or https), authority(host and optional port), and path
// Example: http://myhost:8080/ResourceGroup/ResourceName (http://<authority>/<path>)
...
type Addressable struct {
	// Hostname is deprecated as of v0.6 and replaced with URL. It will be removed in v0.8 or later.
	DeprecatedHostname string `json:"hostname,omitempty"`
	// URL currently supports scheme(http or https), authority(host and optional port), and path.
	// Example: http://myhost:8080/ResourceGroup/ResourceName (http://<authority>/<path>).
	URL *apis.URLRef `json:"url,omitempty"`
}
...
func (t *AddressableType) Populate() {
	t.Status = AddressStatus{
		&Addressable{
			// Populate ALL fields
			DeprecatedHostname: "this is not empty",
			URL:                apis.ParseURLRef("myscheme://myhost:myport/mypath?myquery"),
		},
	}
}
