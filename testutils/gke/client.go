...
func (gsc *sdkClient) GetOperation(project, region, zone, opName string) (*container.Operation, error) {
	...
	clusterFullPath := fmt.Sprintf("projects/%s/locations/%s/operations/%s", project, location, opName)
	return gsc.Service.Projects.Locations.Operations.Get(clusterFullPath).Do()
}
