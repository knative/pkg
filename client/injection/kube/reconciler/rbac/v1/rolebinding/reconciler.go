...
// controller reconciling v1.RoleBinding.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.RoleBinding. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.RoleBinding) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.RoleBinding.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.RoleBinding. Any changes
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.RoleBindings(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1.RoleBinding, desired *v1.RoleBinding) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.RbacV1().RoleBindings(desired.Namespace)
		...
		updater := r.Client.RbacV1().RoleBindings(existing.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.RoleBinding) (*v1.RoleBinding, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.RoleBinding, reconcileEvent reconciler.Event) (*v1.RoleBinding, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
