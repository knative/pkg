...
func ExampleChooseSubset_selectOne() {
	...
	tasks := sets.New("task1", "task2", "task3")
...
func ExampleChooseSubset_selectMany() {
	...
	tasks := sets.New("task1", "task2", "task3", "task4", "task5")
...
func TestChooseSubset(t *testing.T) {
	tests := []struct {
		...
		name:    "return all",
		from:    sets.New("sun", "moon", "mars", "mercury"),
		target:  "a target!",
		wantNum: 4,
		want:    sets.New("sun", "moon", "mars", "mercury"),
	}, {
		name:    "subset 1",
		from:    sets.New("sun", "moon", "mars", "mercury"),
		target:  "a target!",
		wantNum: 2,
		want:    sets.New("mercury", "moon"),
	}, {
		name:    "subset 2",
		from:    sets.New("sun", "moon", "mars", "mercury"),
		target:  "something else entirely",
		wantNum: 2,
		want:    sets.New("mercury", "mars"),
	}, {
		name:    "select 3",
		from:    sets.New("sun", "moon", "mars", "mercury"),
		target:  "something else entirely",
		wantNum: 3,
		want:    sets.New("mars", "mercury", "sun"),
	}}
...
func TestCollisionHandling(t *testing.T) {
	...
	hd := buildHashes(sets.New(key1, key2), target)
