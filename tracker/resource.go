...
//
// The intended use is creating a Reconciler:
//     resourceInformer := duck.NewResourceInformer(ctx)
//     r := &Reconciler{ ... }
//     impl := controller.NewImpl(r, r.Logger, ReconcilerName)
//     // resourceTracker is used to notify this reconciler when a tracked resource changes.
//     resourceTracker: resourceInformer.NewTracker(impl.EnqueueKey, controller.GetTrackerLease(ctx))
type ResourceInformer interface {
	NewTracker(callback func(string), lease time.Duration) ResourceTracker
}

// ResourceTracker is a tracker capable of tracking arbitrary Resources.
//
// The intended use is inside a reconciler's reconcile loop:
...
//     if err = track(utils.ObjectRef(broker, brokerGVK)); err != nil {
//         return err
//     }
//     ...
